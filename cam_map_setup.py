...
def create_full_scene_setup():
    # --- List all the files name
    fileshortnames = list_files(path)
    # --- Create camera
    camera = mc.createNode('camera', shared=True, n='projection_cam')
    # --- Loop for every file in the list
    create_setup(fileshortnames, camera)

# ----------------------------------------------------------------------------------------------------------------------
def create_setup(fileslist, camera):
    # --- Loop for every file in the list
...
def list_files(path):
    # --- Format path
    formated_path = ('/').join(path.split('\\'))

    # --- List long path of all files in the path directory
    filelongnames = glob.glob(formated_path+'/*diff.png')

    # --- Format long paths to get names
    fileshortnames = [obj.split('\\')[-1] for obj in filelongnames]
